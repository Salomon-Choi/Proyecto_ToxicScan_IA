...
reddit = praw.Reddit(client_id = open('client_id.txt').read(),              # your client id
                      client_secret = open('client_secret.txt').read(),        #your client secret
                      user_agent = open('user_agent.txt').read(),                      #user agent name,
                      username = open('username.txt').read(),           # your reddit username
                      password = open('password.txt').read(),      )     # your reddit password
...
#Get the post ID:
reddit_post_url = "https://www.reddit.com/r/oddlysatisfying/comments/1813mvq/construction_worker_throwing_mortar_like_a_king/"
...
if not json_ids['detail']:
    ...
    for count,item in enumerate(step_counter):
        ...
        try:
            ...
            comment_list = [thing['body'] for thing in json_comments['detail'] if thing['body'] not in remove_words]
